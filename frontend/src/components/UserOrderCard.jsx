...
function UserOrderCard({ data }) {
    ...
    const handleRating = async (itemId, rating) => {
        if (selectedRating[itemId] && selectedRating[itemId] === rating) return;
        try {
    ...
    return (
        <div className='bg-white rounded-lg shadow p-4 space-y-4'>
            ...
            {data?.shopOrders?.map((shopOrder) => (
                <div className='border rounded-lg p-3 bg-[#fffaf7] space-y-3' key={shopOrder._id}>
                        ...
                        {shopOrder?.shopOrderItems?.map((item) => (
                            <div key={item.item._id} className='shrink-0 w-40 border rounded-lg p-2 bg-white'>
                                ...
                                {shopOrder.status == "delivered" && <div className='flex space-x-1 mt-2'>
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        <button className={`text-lg ${selectedRating[item.item._id] >= star ? 'text-yellow-400' : 'text-gray-400'}`} key={star} onClick={() => handleRating(item.item._id, star)}>★</button>
                                    ))}
                                </div>}
